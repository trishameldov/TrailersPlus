...
export function initFleetCF() {
    const form = document.getElementById('fleet-cf')
    ...
    if(form) {
        ...
        async function submitForm(event) {
            ...
            // Renames the quantity field for the endpoint
            jsonData["excepted_fleet"] = jsonData["quantity"];
            delete jsonData["quantity"];
            
            // Removes the <p> and </br> tags from the strings.
            jsonData["trailer_type"] = jsonData["trailer_type"].replace(/<[^>]*>/g, "");
            jsonData["trailer_length"] = jsonData["trailer_length"].replace(/<[^>]*>/g, "");
            jsonData["excepted_fleet"] = jsonData["excepted_fleet"].replace(/<[^>]*>/g, "");

            const response = await fetch(`${config.env.apiURL}/forms/fleet`, {
                method: 'POST',
            ...
            if(response.status === 201) {
                ...
                const inputs = document.getElementById('fleet-cf').getElementsByTagName("input");
